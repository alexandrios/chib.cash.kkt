...
namespace chib.cash.shtrih64
{
    public class Config
    {
        ...
        // Тип связи с компьютером: 0 - локально по COM-порту(в том числе виртуальному), 6 - TCP сокет
        // Для ConnectionType = 0 необходимо указать параметр PortName = < Номер COM-порта>
        // Для ConnectionType = 6 необходимо указать параметры IPAddress = < xxx.xxx.xxx.xxx > и TCPPort = <Номер порта>
        ...
        private void ReadIniFile()
        {
            ...
            ConnectionType = GetIniParamInt("ConnectionType", 0);
